...
def get_subcomplex_pooling_layer(embedding_dim=94, aggregation: str = "mean"):
    return nn.ModuleDict(
        {
            "x_0": SubComplexPool(
                low_rank=0, high_rank=1, return_low_rank=True, aggregation=aggregation
            ),
            "x_1": SubComplexPool(
                low_rank=0, high_rank=1, return_low_rank=False, aggregation=aggregation
            ),
        }
    )


def get_subcomplex_cross_pool(aggregation="mean"):
    return nn.ModuleDict(
        {"x_2": SubComplexCrossPool(low_rank=0, high_rank=2, aggregation=aggregation)}
    )


def get_subcomplex_embed_layer(
    embedding_dim=94, aggregation: str = "concatenate", activation: str = "relu"
):
    return nn.ModuleDict(
        {
            "x_0_1": MergeNode(
                [
                    SubComplexBroadcastLow(low_rank=0, high_rank=1),
                    SubComplexDistanceMarkingEmbed(
                        low_rank=0, high_rank=1, embed_dim=embedding_dim
                    ),
                ],
...
def build_subcomplex_ensemble_model(
    ...
    device: str,
    output_ranks_cin: List[int] = [0, 1],
    output_ranks_subgraphs: List[int] = [0, 1],
